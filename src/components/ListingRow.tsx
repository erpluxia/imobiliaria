...
export default function ListingRow({ p }: { p: Property }) {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-[360px_1fr_220px]">
        ...
        <div className="p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <Link to={`/imovel/${p.id}`} className="font-semibold text-gray-900 hover:underline line-clamp-2">{p.title}</Link>
        ...
        <div className="border-t md:border-t-0 md:border-l p-4 flex md:flex-col items-center md:items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to={`/imovel/${p.id}`} className="bg-indigo-600 text-white rounded-md px-4 py-2 text-sm hover:bg-indigo-700">Mensagem</Link>
          </div>
        </div>
      </div>
    </article>
